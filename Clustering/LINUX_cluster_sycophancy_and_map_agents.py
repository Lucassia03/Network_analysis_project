...
# --------------------------------------------------
# Keyword per stimare sicofantia / adulazione
# --------------------------------------------------
# Nota: questa lista misura soprattutto linguaggio approvativo,
# adulatorio o eccessivamente concorde.
# Non misura "sicofantia" nel senso storico di delazione/calunnia.
...
# --------------------------------------------------
# Nomi opzionali dei 7 cluster interpretati
# --------------------------------------------------
# Questi nomi valgono solo se il clustering produce cluster simili
# a quelli già osservati nei tuoi file precedenti.
# Se cambi dati, preprocessing o numero di cluster, vanno ricontrollati.
...
def check_table_exists(db_path: str, table_name: str) -> None:
    """
    Controlla che una tabella esista nel database SQLite.
    """
...
def load_comments(db_path: str, table_name: str) -> pd.DataFrame:
    """
    Carica i commenti dal database SQLite.
    Richiede una tabella con almeno:
    - id
    - post_id
    - body
    """
...
def save_table(
    ...
    """
    Salva un dataframe dentro una tabella SQLite.
    Se la tabella esiste già, viene sovrascritta.
    """
...
def clean_text(text: str) -> str:
    """
    Pulisce il testo del commento per TF-IDF e clustering.
    """
...
def normalize_for_keyword_matching(text: str) -> str:
    """
    Normalizza il testo originale in modo più leggero rispetto a clean_text.
    Serve per cercare frasi tipo "great question", "thank you", "well said".
    """
...
def compute_sycophancy_score_for_clusters(
    ...
    """
    Calcola uno score di possibile sicofantia per ogni cluster.

    Usa tre segnali:

    1. Quota di commenti del cluster che contengono almeno una keyword.
    2. Numero totale di keyword ogni 100 commenti.
    3. Overlap tra keyword e top terms del cluster.

    Il risultato non è una diagnosi certa, ma una classifica dei cluster
    più probabilmente caratterizzati da linguaggio approvativo/adulatorio.
    """
    ...
    for cluster_id, group in df.groupby(cluster_col):
        ...
        comment_hit_rate = (
            keyword_comment_hits / n_comments
        ...
        # Score sintetico.
        # Puoi cambiarne i pesi se vuoi dare più importanza alle top words
        # o alla frequenza nei commenti reali.
        sycophancy_score = (
        ...
        rows.append({
            ...
            "keyword_comment_hits": keyword_comment_hits,
            "comment_hit_rate": comment_hit_rate,
            "comment_hit_rate_pct": comment_hit_rate * 100,
            "total_keyword_hits": total_keyword_hits,
            "keyword_hits_per_100_comments": keyword_hits_per_100_comments,
            "top_term_overlap_score": top_term_overlap_score,
